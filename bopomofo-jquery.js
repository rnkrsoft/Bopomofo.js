...
(function($, window) {
	$.bopomofo = function(){
		...
		/**
		 * 将拼音转换方法转换为闭包能访问的方法，因为pinyin在闭包中存在
		 ...
		 */
		function _pinyin(words, toneType, upper, cap, split) {
			return pinyin(words, toneType, upper, cap, split);
		}
		/**
		...
		Bopomofo.prototype = {
			 ...
			 * @return 拼音
			 */
			pinyin: function(words, toneType, upper, cap, split){
				return _pinyin(words, toneType, upper, cap, split);
			}
		};
		var instance = new Bopomofo();
		return instance;
	};
})(jQuery, window);